 ...
 * @description 统一管理我的证书、自定义资料、本地数据和协会资料库入口。
 */
...
import ProfileIssuedCertificatesPanel from './components/ProfileIssuedCertificatesPanel/ProfileIssuedCertificatesPanel.vue'
import ProfileLocalDataPanel from './components/ProfileLocalDataPanel/ProfileLocalDataPanel.vue'

const { t } = useI18n()
const route = useRoute()
const uiStore = useUiStore()
const validProfileTabs: ProfileTab[] = ['certificates', 'customAssets', 'localData', 'catalog']
...
/**
 * @description: 判断个人中心页签是否合法
 * @description 兼容旧版本持久化的 activeDraft，避免升级后落到无效页签。
 * @param {unknown} value - 待判断值
 * @return {value is ProfileTab} 是否为当前支持的页签
 */
function isProfileTab(value: unknown): value is ProfileTab {
  return typeof value === 'string' && validProfileTabs.includes(value as ProfileTab)
}

if (!isProfileTab(uiStore.profileTab)) {
  uiStore.setProfileTab('certificates')
}
  ...
  <ResponsivePageShell :title="t('profile.title')" :subtitle="t('profile.subtitle')">
    <v-card variant="flat" class="profile-card">
      ...
      <v-card-text class="profile-card__content">
        ...
        <ProfileIssuedCertificatesPanel v-else-if="uiStore.profileTab === 'certificates'" />
        <ProfileLocalDataPanel v-else-if="uiStore.profileTab === 'localData'" />
      </v-card-text>
    </v-card>
  </ResponsivePageShell>
</template>
