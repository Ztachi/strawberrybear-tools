...
任何页面级、详情页、抽屉列表、虚拟列表等自有纵向滚动容器，只要内容可能超过可视区域，都必须
接入 `useMainWindowUiStore().registerBackToTop(...)`。滚动超过约 `200px` 时显示右下角返回顶部
入口，点击后滚动当前容器到顶部，组件卸载时必须注销注册。已有“定位当前项”语义的列表可以同时
注册 `registerLocateCurrent(...)`，但不能用原生滚动条替代长页面的返回顶部入口。
