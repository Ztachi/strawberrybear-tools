...
函数注释不能替代函数内部注释。函数体内的关键状态切换、异步流程顺序、错误兜底、平台分支、
mock/调试分支、持久化读写、事件回调转换、资源释放和会影响 UI 状态的副作用，都必须在对应
代码行附近补充中文注释，说明“为什么这样做”或“这一步影响什么状态”。简单赋值、显而易见的
返回值和重复模板代码不需要机械注释；但不能只给函数写 JSDoc 后让关键逻辑裸奔。
