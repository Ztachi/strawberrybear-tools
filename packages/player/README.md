...
`play()`、`seek()`、`pause()`、`resume()` 和 `stop()` 使用同一请求序号。旧操作的完成或失败晚于新操作时会被忽略，不能覆盖当前媒体、播放位置或状态。加载中 seek 会取代旧加载；端口没有通过回调确认播放时，进入可恢复的暂停态。

这些保证只控制公共状态回写。音频端口仍需用自己的会话令牌取消旧 I/O 或音频准备任务，避免旧任务继续发声；平台事件也必须在端口确认属于当前会话后回灌。
